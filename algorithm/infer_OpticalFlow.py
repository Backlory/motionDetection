...
class Inference_OpticalFlow():
    ...
    def run_test(self, fps_target=30, dataset = 'u'):
        ...
        while(True):
            ...
            # ==============================================↓↓↓↓
            flo_ten, flo, flo_activate = self.__call__(img_t0, img_t1_warp, moving_mask)
    ...
    def __call__(self, img_base, img_warped, moving_mask, last_flow = None):
        ...
        #################################
        # RAFT 光流
        flow_low, flow_up, coords0, coords1 = self.model(image1, image2, Masks, iters=10, test_mode=True, flow_init = last_flow)
        last_flow = flow_low.detach()
        toc(tp, "RAFT", mute=False); tp = tic()
        
        # 获取光流后处理
        flo_ten = flow_up[0].permute(1,2,0).cpu().numpy()

        flo = flow_viz.flow_to_image(flo_ten)
        ...
        return flo_ten, flo, flo_activate
