...
class Mask_RAFT(RAFT):
    def __init__(self, gridLength = 32, gridEdgeNum = 20):
    ...
    def forward(self, image1, image2, Mask, iters=12, flow_init=None, upsample=True, test_mode=False):
        ...
        # 计算corr
        corr_fn = MaskCorrBlock(fmap1, fmap2, radius=self.args.corr_radius, Mask=Mask)
        ...
        with autocast(enabled=self.args.mixed_precision):
            ...
            h2 = w2 = self.gridLength * self.gridEdgeNum
            cnet = cnet * F.interpolate(Mask, (h2, w2), mode="nearest")
        ...
        for i in range(self.gridEdgeNum):
            for j in range(self.gridEdgeNum):
                if Mask[0, 0, i, j] > 0:
                    y = i * self.gridLength
                    x = j * self.gridLength
                    image1_fea_patches.append(image1[:, :, y:y+self.gridLength+6, x:x+self.gridLength+6])
                    image2_fea_patches.append(image2[:, :, y:y+self.gridLength+6, x:x+self.gridLength+6])
                    fea_Position.append([i,j])
        if len(image1_fea_patches) == 0:
            return
        else:
            image1_fea_patches = torch.cat(image1_fea_patches)
            image2_fea_patches = torch.cat(image2_fea_patches)
            '''