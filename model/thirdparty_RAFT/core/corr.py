...
class MaskCorrBlock:
    def __init__(self, fmap1, fmap2, num_levels=4, radius=4, Masks=[None, None], gridLength=4):
        ...
        h_num = h1 // gridLength
        w_num = w1 // gridLength
        corr = corr.reshape(h_num, gridLength, w_num, gridLength, dim, h2, w2)
        corr = corr.permute([0,2,1,3,4,5,6])
        ...
        #corr = corr * Mask_big.view(1,80,80,1,1,1)
    ...
    def __call__(self, coords):
        ...
        h1 = h_num*gridLength
        w1 = w_num*gridLength
        h_num, w_num, _, gridLength, gridLength = coords.shape
        ...
        for i in range(self.num_levels):    #每层都要取
            ...
            w1_layer = w1 // (2**i)
            corr = self.corr_pyramid[i]     #n个点，单通道，与其余64*64个点的corr值
            corr = corr.reshape(h_num, w_num, gridLength, gridLength, h1_layer, w1_layer)
            #corr = corr.view(1, h1, w1, h1, w1) #前为t0，后为t1
